...
ssh_creation()
{
    ...
    command='rm -f /home/$dest_user/.ssh/id_rsa /home/$dest_user/.ssh/id_rsa.pub /home/$dest_user/.ssh/known_hosts && ssh-keygen -t rsa'
    ssh ${options} -i ${HOME}/.ssh/identities/${client}.pem -t ${user}@${host} "sudo -u ${dest_user} -i ${command}"  
